...
<div class="card">
    <div class="card-header">
        ...
        <span class="badge badge-primary" title="Reputasi">
            <i class="fas fa-award mr-1"></i> {{ $tanya->user_reputasi ?: 0}}
        </span>
            ...
            <table class="table table-borderless table-sm">
                <tr>
                    <td class="text-center">
                        <a href="{{ route('votepertanyaan.up', $tanya->id) }}" class="btn btn-sm btn-warning {{ $tanya->user_id == Auth::user()->id ? 'disabled' : '' }} mb-1" title="Vote Up / Cendol">
                            <i class="fas fa-chevron-up" aria-hidden="true"></i>
                        </a>
                        <br>
                        <span class="text-muted">{{ $tanya->poinvote }}</span>
                        <br>
                        <a href="{{ route('votepertanyaan.down', $tanya->id) }}" class="btn btn-sm btn-warning {{ $tanya->user_id == Auth::user()->id ? 'disabled' : '' }} mt-1" title="Vote Down / Bata">
                            <i class="fa fa-chevron-down" aria-hidden="true"></i>
                        </a>
                    </td>
                    <td>
                        <table class="table table-borderless table-sm small" style="margin-top: -5px">
                            @forelse ($komentar as $i => $comment)
                            <tr style="border-bottom: 1px solid rgba(0,0,0,.1)">
                                <td>{{ $i+1}}.</td>
                                <td width="90%">{{ $comment->isi}}</td>
                                <td>
                                    <span class="badge badge-info" title="{{ $comment->email }}">
                                        <i class="fas fa-user mr-1"></i> {{ $comment->name }}
                                    </span>
                                </td>
